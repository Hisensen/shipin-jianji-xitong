"""V7 版生成器：极简 AI 科技风（V5 同款丰富数据 + 顶部进度条）。

用法:
    .venv/bin/python remotion/make_cues_v7.py "/path/to/video.mp4"
"""
...
def main() -> None:
    if len(sys.argv) < 2:
        sys.exit("用法: python make_cues_v7.py <video>")
    ...
    images_dir = ROOT / "public" / "images_v7"
    ...
    for i, d in enumerate(decisions):
        ...
        if provider:
            d["imageSrc"] = f"images_v7/{dst.name}"
    ...
    for i, ((start, end, text), d, q) in enumerate(zip(segments, decisions, quote_emph)):
        ...
        if not cue["isQuote"]:
            if f == "image":
                ...
                cue["imageMode"] = "fullscreen"  # V7：全屏 B-roll
            elif f == "callout":
    ...
    print("[9/9] 写 src/realCuesV7.ts")
    cues_ts = ROOT / "src" / "realCuesV7.ts"
    payload = {
        "videoSrc": "source.mp4",
        ...
        # V7 极简 AI 科技：不用 neonMode 也不用 emotionMode，用专属 minimalAIMode
        "minimalAIMode": True,
        "width": vid_w,
        "height": vid_h,
    }
    body = (
        "// Auto-generated by make_cues_v7.py — do not edit by hand.\n"
        "import type { SubtitleProps } from './SubtitleWithImages';\n\n"
        "export const realPropsV7: SubtitleProps = "
        + json.dumps(payload, ensure_ascii=False, indent=2)
        + " as unknown as SubtitleProps;\n"
    )
    cues_ts.write_text(body, encoding="utf-8")

    print(f"\nV7 完成。{len(beat_frames)} 重音 / {n_quotes} 金句 / 全屏 B-roll / 霓虹配色。")
